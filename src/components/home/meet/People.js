...
import { EllipsisVertical, MicOff, Video } from 'lucide-react-native';
      ...
      >
        ...
        {!person?.micOn && (
          <View style={peopleStyles.muted}>
            <MicOff color={'white'} size={RFValue(10)}/>

          </View>
        )}
        <View style={peopleStyles.ellipsis}>
          <EllipsisVertical color={'white'} size={RFValue(14)}/>
          </View>
          {othersCount > 0 && index == visiblePeople?.length - 1 && (
            <TouchableOpacity
            style={[peopleStyles.others]}
            activeOpacity={0.8}>
              <Text style={peopleStyles.othersText}>{othersCount} others</Text>
            </TouchableOpacity>
          )}
      </View>
    );
